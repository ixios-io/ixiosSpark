...
// Client is a wrapper around rpc.Client that implements ixiosSpark-specific functionality.
//
// If you want to use the standardized Ixios RPC functionality, use client.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg ixiosSpark.CallMsg) (*types.AccessList, uint64, string, error) {
...
// the message call.
// Please use client.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg ixiosSpark.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
//
// Please use client.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContractWithBlockOverrides(ctx context.Context, msg ixiosSpark.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount, blockOverrides BlockOverrides) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "eth_call", toCallArg(msg),
...
// GCStats retrieves the current garbage collection stats from a ixiosSpark node.
...
// MemStats retrieves the current memory stats from a ixiosSpark node.
...
// GetNodeInfo retrieves the node info of a ixiosSpark node.
