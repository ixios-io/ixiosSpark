...
between ixios nodes by simply copying.
...
`,
			},
			{
				...
				Description: `
    ixiosSpark account update <address>
...
For non-interactive use the password can be specified with the --password flag:
    ...
    ixiosSpark account update [options] <address>
...
`,
			},
			{
				...
				ArgsUsage: "<keyFile>",
				Description: `
    ixiosSpark account import <keyfile>
...
For non-interactive use the password can be specified with the -password flag:
    ...
    ixiosSpark account import [options] <keyfile>

Note:
As you can directly copy your encrypted accounts to another ixios instance,
...
func makeAccountManager(ctx *cli.Context) *accounts.Manager {
	...
	if isEphemeral {
		Fatalf("Can't use ephemeral directory as keystore path")
	}
...
func unlockAccount(ks *keystore.KeyStore, address string, i int, passwords []string) (accounts.Account, string) {
	...
	for trials := 0; trials < 3; trials++ {
		...
		password := getPassPhraseWithList(prompt, false, i, passwords)
...
func accountUpdate(ctx *cli.Context) error {
	...
	for _, addr := range ctx.Args().Slice() {
		...
		newPassword := getPassPhraseWithList("Please give a new password. Do not forget this password.", true, 0, nil)
...
func importWallet(ctx *cli.Context) error {
	if ctx.Args().Len() != 1 {
		Fatalf("keyfile must be given as the only argument")
	}
	...
	ks := backends[0].(*keystore.KeyStore)
	passphrase := getPassPhraseWithList("", false, 0, MakePasswordList(ctx))

	acct, err := ks.ImportKey(keyJSON, passphrase)
	if err != nil {
		Fatalf("%v", err)
	}
...
func accountImport(ctx *cli.Context) error {
	...
	ks := backends[0].(*keystore.KeyStore)
	passphrase := getPassPhraseWithList("Your new account is locked with a password. Please give a password. Do not forget this password.", true, 0, MakePasswordList(ctx))
