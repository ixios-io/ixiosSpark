...
func ImportHistory(chain *core.BlockChain, db kvdb.Database, dir string, network string) error {
...
func ImportPreimages(db kvdb.Database, fn string) error {
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db kvdb.Database, fn string) error {
...
func ExportSnapshotPreimages(chaindb kvdb.Database, snaptree *snapshot.Tree, fn string, root common.Hash) error {
	log.Info("Exporting preimages", "file", fn)
...
func ImportLDBData(db kvdb.Database, f string, startIndex int64, interrupt chan struct{}) error {
...
func ExportChaindata(fn string, kind string, iter ChainDataIterator, interrupt chan struct{}) error {
	...
	if err := rlp.Encode(writer, &exportHeader{
		...
		Version:  0,
		Kind:     kind,
		UnixTime: uint64(time.Now().UnixMilli()),
	}); err != nil {
		return err
	}
