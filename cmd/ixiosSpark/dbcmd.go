...
func showLeveldbStats(db kvdb.KeyValueStater) {
...
func importLDBdata(ctx *cli.Context) error {
	...
	db := MakeChainDatabase(ctx, stack, false)
	defer db.Close()
	return ImportLDBData(db, fName, int64(start), stop)
}
...
func (iter *preimageIterator) Next() (byte, []byte, []byte, bool) {
	for iter.iter.Next() {
		...
		if bytes.HasPrefix(key, rawdb.PreimagePrefix) && len(key) == (len(rawdb.PreimagePrefix)+common.HashLength) {
			return OpBatchAdd, key, iter.iter.Value(), true
		}
	}
...
type snapshotIterator struct {
	init    bool
	account kvdb.Iterator
	storage kvdb.Iterator
}

func (iter *snapshotIterator) Next() (byte, []byte, []byte, bool) {
	if !iter.init {
		iter.init = true
		return OpBatchDel, rawdb.SnapshotRootKey, nil, true
	}
	for iter.account.Next() {
		key := iter.account.Key()
		if bytes.HasPrefix(key, rawdb.SnapshotAccountPrefix) && len(key) == (len(rawdb.SnapshotAccountPrefix)+common.HashLength) {
			return OpBatchAdd, key, iter.account.Value(), true
		}
	}
	for iter.storage.Next() {
		key := iter.storage.Key()
		if bytes.HasPrefix(key, rawdb.SnapshotStoragePrefix) && len(key) == (len(rawdb.SnapshotStoragePrefix)+2*common.HashLength) {
			return OpBatchAdd, key, iter.storage.Value(), true
		}
	}
	return 0, nil, nil, false
}
...
var chainExporters = map[string]func(db kvdb.Database) ChainDataIterator{
	"preimage": func(db kvdb.Database) ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db kvdb.Database) ChainDataIterator {
...
func exportChaindata(ctx *cli.Context) error {
	...
	db := MakeChainDatabase(ctx, stack, true)
	defer db.Close()
	return ExportChaindata(ctx.Args().Get(1), kind, exporter(db), stop)
}
