...
import (
	...
	"github.com/ixios-io/ixiosSpark/common/fdlimit"
	...
	"github.com/ixios-io/ixiosSpark/ixios/tracers"
	"github.com/ixios-io/ixiosSpark/kvdb"
	"github.com/ixios-io/ixiosSpark/kvdb/remotedb"
	...
	"github.com/ixios-io/ixiosSpark/p2p/nat"
	"github.com/ixios-io/ixiosSpark/p2p/netutil"
	...
	"github.com/ixios-io/ixiosSpark/triedb/hashdb"
	"github.com/ixios-io/ixiosSpark/triedb/pathdb"
...
var (
	...
	MainnetFlag = &cli.BoolFlag{
		Name:     "mainnet",
		Usage:    "Ixios Mainnet: main network",
		Category: flags.EthCategory,
	}
	AetherForgeFlag = &cli.BoolFlag{
		Name:     "aetherForge",
		Usage:    "AetherForge network: validator test network",
		Category: flags.EthCategory,
	}
	AetherBloomFlag = &cli.BoolFlag{
		Name:     "aetherBloom",
	...
	MiningEnabledFlag = &cli.BoolFlag{
		Name:     "sealer.enabled",
		Usage:    "Enable sealing",
		Category: flags.MinerCategory,
	}
	MinerGasLimitFlag = &cli.Uint64Flag{
		Name:     "sealer.gaslimit",
		Usage:    "Target gas ceiling for mined blocks",
		Value:    ethconfig.Defaults.Miner.GasCeil,
		Category: flags.MinerCategory,
	}
	MinerGasPriceFlag = &flags.BigFlag{
		Name:     "sealer.gasprice",
		Usage:    "Minimum gas price for mining a transaction",
		Value:    ethconfig.Defaults.Miner.GasPrice,
		Category: flags.MinerCategory,
	}
	MinerEtherbaseFlag = &cli.StringFlag{
		Name:     "sealer.etherbase",
		Usage:    "0x prefixed public address for block mining rewards",
		Category: flags.MinerCategory,
	}
	MinerExtraDataFlag = &cli.StringFlag{
		Name:     "sealer.extradata",
		Usage:    "Block extra data set by the sealer (default = client version)",
		Category: flags.MinerCategory,
	}
	MinerRecommitIntervalFlag = &cli.DurationFlag{
		Name:     "sealer.recommit",
		Usage:    "Time interval to recreate the block being mined",
		Value:    ethconfig.Defaults.Miner.Recommit,
		Category: flags.MinerCategory,
	}
	MinerNewPayloadTimeout = &cli.DurationFlag{
		Name:     "sealer.newpayload-timeout",
	...
	EnableBroadcastFlag = &cli.BoolFlag{
		Name:     "enable-broadcast",
		Usage:    "Enable broadcasting blocks to all peers instead of sqrt subset",
		Category: flags.NetworkingCategory,
	}
	...
	HttpHeaderFlag = &cli.StringSliceFlag{
		Name:     "header",
		Aliases:  []string{"H"},
		Usage:    "Pass custom headers to the RPC server when using --" + RemoteDBFlag.Name + " or the ixiosSpark attach console. This flag can be given multiple times.",
		Category: flags.APICategory,
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(AetherForgeFlag.Name) {
			return filepath.Join(path, "aetherForge")
		}
...
// 1. --bootnodes flag
// 2. Config file
// 3. Network preset flags (eg --aetherSeed)
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.IsSet(BootnodesFlag.Name) {
		...
		case ctx.Bool(AetherBloomFlag.Name):
			urls = params.AetherBloomBootnodes
		}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `ixiosSpark account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "Ixios", ethPeers, "total", cfg.MaxPeers)
	...
	if ctx.IsSet(EnableBroadcastFlag.Name) {
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	// Lightweight KDF is always disabled
	cfg.UseLightweightKDF = false
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(AetherForgeFlag.Name) && cfg.DataDir == node.DefaultDataDir():
...
func setMiner(ctx *cli.Context, cfg *sealer.Config) {
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	syncMode := *flags.GlobalTextMarshaler(ctx, SyncModeFlag.Name).(*downloader.SyncMode)
	if ctx.IsSet(SyncModeFlag.Name) {
		if syncMode == downloader.FullSync {
			cfg.SyncMode = downloader.FullSync
		} else {
			Fatalf("Unsupported syncmode")
		}
	}
	...
	if ctx.String(GCModeFlag.Name) == "archive" && cfg.TransactionHistory != 0 {
		...
		log.Info("Disabled transaction unindexing for archive node")
	}
	...
	if ctx.IsSet(VMEnableDebugFlag.Name) || ctx.IsSet(DeveloperFlag.Name) {
	...
	case ctx.Bool(AetherForgeFlag.Name):
		...
		cfg.Genesis = core.DefaultAetherForgeGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.AetherForgeGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
		...
		// setEtherbase has been called above, configuring the sealer address from command line flags.
	...
	default:
		// If no flags are set, default to mainnet
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1
		}
		cfg.Genesis = core.DefaultGenesisBlock()
...
// RegisterEthService adds an Ixios client to the stack.
// The second return value is the full node instance.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *ixios.Ixios) {
	backend, err := ixios.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Ixios L1 service: %v", err)
	}
...
func RegisterFullSyncTester(stack *node.Node, eth *ixios.Ixios, target common.Hash) {
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node, readonly bool) kvdb.Database {
	var (
...
func tryMakeReadOnlyDatabase(ctx *cli.Context, stack *node.Node) kvdb.Database {
...
func MakeChain(ctx *cli.Context, stack *node.Node, readonly bool) (*core.BlockChain, kvdb.Database) {
	var (
...
func MakeTrieDatabase(ctx *cli.Context, disk kvdb.Database, preimage bool, readOnly bool, isVerkle bool) *triedb.Database {
