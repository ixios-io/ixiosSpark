		...
		Subcommands: []*cli.Command{
			{
				...
				Description: `
ixiosSpark snapshot prune-state <state-root>
...
`,
			},
			{
				...
				ArgsUsage: "<root>",
				Action:    verifyState,
				Flags:     flags.Merge(NetworkFlags, DatabaseFlags),
				Description: `
ixiosSpark snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Flags:     flags.Merge(NetworkFlags, DatabaseFlags),
				Description: `
ixiosSpark snapshot check-dangling-storage <state-root> traverses the snap storage
...
`,
			},
			{
				...
				Flags:     flags.Merge(NetworkFlags, DatabaseFlags),
				Description: `
ixiosSpark snapshot inspect-account <address | hash> checks all snapshot layers and prints out
...
`,
			},
			{
				...
				Flags:     flags.Merge(NetworkFlags, DatabaseFlags),
				Description: `
ixiosSpark snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Flags:     flags.Merge(NetworkFlags, DatabaseFlags),
				Description: `
ixiosSpark snapshot traverse-rawstate <state-root>
...
`,
			...
			{
				Name:      "dump",
				Usage:     "Dump a specific block from storage (same as 'ixiosSpark dump' but using snapshots)",
				...
				Description: `
This command is semantically equivalent to 'ixiosSpark dump', but uses the snapshots
...
func verifyState(ctx *cli.Context) error {
	...
	triedb := MakeTrieDatabase(ctx, chaindb, false, true, false)
	defer triedb.Close()
...
func dumpState(ctx *cli.Context) error {
	...
	triedb := MakeTrieDatabase(ctx, db, false, true, false)
	defer triedb.Close()
...
func snapshotExportPreimages(ctx *cli.Context) error {
	...
	return ExportSnapshotPreimages(chaindb, snaptree, ctx.Args().First(), root)
}
