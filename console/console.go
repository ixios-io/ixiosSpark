...
import (
	...
	"github.com/ixios-io/ixiosSpark/internal/jsre"
	"github.com/ixios-io/ixiosSpark/internal/jsre/deps"
	"github.com/ixios-io/ixiosSpark/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Ixios requests through
...
func (c *Console) initWeb3(bridge *bridge) error {
	...
	c.jsre.Do(func(vm *goja.Runtime) {
		...
		_, err := vm.RunString(`
        try {
            var web3 = new Web3(_consoleWeb3Transport);
        } catch (e) {
            console.log('Error initialising Web3:', e);
            throw e;  // Re-throw the error so it can be caught in Go
        }
    `)
		if err != nil {
			panic("failed to initialise web3.js: " + err.Error())
		}

		// verify web3 object exists
		web3Obj := vm.Get("web3")
		if web3Obj == nil || goja.IsUndefined(web3Obj) {
			panic("'web3' object is not defined after initialisation\n")
		}
...
func (c *Console) initAdmin(vm *goja.Runtime, bridge *bridge) {
	...
	if admin := getObject(vm, "admin"); admin != nil {
		err = admin.Set("sleepBlocks", jsre.MakeCallback(vm, bridge.SleepBlocks))
		if err != nil {
			panic("sleepBlocks failed: " + err.Error())
		}
		err = admin.Set("sleep", jsre.MakeCallback(vm, bridge.Sleep))
		if err != nil {
			panic("sleep failed: " + err.Error())
		}
		err = admin.Set("clearHistory", c.clearHistory)
		if err != nil {
			panic("clearHistory failed: " + err.Error())
		}
	}
}
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jeth := vm.NewObject()
	err = vm.Set("jeth", jeth)
	if err != nil {
		panic("Failed to set jeth object: " + err.Error())
	}
	err = jeth.Set("openWallet", personal.Get("openWallet"))
	if err != nil {
		panic("Failed to set openWallet object: " + err.Error())
	}
...
// Welcome shows summary of current ixiosSpark instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the ixiosSpark JavaScript console!\n\n"
	...
	if res, err := c.jsre.Run(`
		...
		message += "at block: " + eth.blockNumber + " (" + new Date(eth.getBlock(eth.blockNumber).timestamp) + ")\n";
		try {
