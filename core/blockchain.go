...
import (
	...
	"github.com/ixios-io/ixiosSpark/internal/syncx"
...
type CacheConfig struct {
	...
	StateScheme         string        // Scheme used to store states and merkle tree nodes on top
...
type BlockChain struct {
	...
	db            kvdb.Database                    // Low level persistent database to store final content in
...
// available in the database. It initialises the default Ixios Validator
// and Processor.
func NewBlockChain(db kvdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db kvdb.KeyValueWriter, header *types.Header) (*types.Header, bool) {
	...
	delFn := func(db kvdb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) error {
	...
	if nodes > limit || imgs > 4*1024*1024 {
		bc.triedb.Cap(limit - kvdb.IdealBatchSize)
	}
...
func summarizeBadBlock(block *types.Block, receipts []*types.Receipt, config *params.ChainConfig, err error) string {
	...
	remoteVersion, vcs := version.Info()
	platform := fmt.Sprintf("%s %s %s %s", remoteVersion, runtime.Version(), runtime.GOARCH, runtime.GOOS)
	if vcs != "" {
		vcs = fmt.Sprintf("\nVCS: %s", vcs)
	}
	localVersion := fmt.Sprintf("%d.%d.%d%s",
		params.VersionMajor,
		params.VersionMinor,
		params.VersionPatch,
		params.VersionMeta,
