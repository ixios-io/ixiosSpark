...
type ChainIndexer struct {
	chainDb  kvdb.Database       // Chain database to index the data from
	indexDb  kvdb.Database       // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb kvdb.Database, indexDb kvdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
func (c *ChainIndexer) AddCheckpoint(section uint64, shead common.Hash) {
	if !c.lock.TryLock() {
		log.Error("Failed to acquire lock for AddCheckpoint", "section", section)
		return
	}
	defer c.lock.Unlock()
