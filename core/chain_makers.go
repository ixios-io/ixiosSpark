...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db kvdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
	...
	for i := 0; i < n; i++ {
		...
		if err := receipts.DeriveFields(config, block.Hash(), block.NumberU64(), block.Time(), block.BaseFee(), txs); err != nil {
			panic(err)
		}
...
func GenerateChainWithGenesis(genesis *Genesis, engine consensus.Engine, n int, gen func(int, *BlockGen)) (kvdb.Database, []*types.Block, []types.Receipts) {
...
func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	time := parent.Time() + 1 // block time is fixed at 1 second
	header := &types.Header{
...
func makeHeaderChain(chainConfig *params.ChainConfig, parent *types.Header, n int, engine consensus.Engine, db kvdb.Database, seed int) []*types.Header {
...
func makeHeaderChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine, seed int) (kvdb.Database, []*types.Header) {
...
func makeBlockChain(chainConfig *params.ChainConfig, parent *types.Block, n int, engine consensus.Engine, db kvdb.Database, seed int) []*types.Block {
...
func makeBlockChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine, seed int) (kvdb.Database, []*types.Block) {
