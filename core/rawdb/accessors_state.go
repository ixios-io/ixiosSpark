...
func ReadPreimage(db kvdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db kvdb.KeyValueWriter, preimages map[common.Hash][]byte) {
...
func ReadCode(db kvdb.KeyValueReader, hash common.Hash) []byte {
...
func ReadCodeWithPrefix(db kvdb.KeyValueReader, hash common.Hash) []byte {
...
func HasCode(db kvdb.KeyValueReader, hash common.Hash) bool {
	// Try with the prefixed code scheme first, if not then try with legacy
	// scheme.
...
func HasCodeWithPrefix(db kvdb.KeyValueReader, hash common.Hash) bool {
	ok, _ := db.Has(codeKey(hash))
	return ok
}

// WriteCode writes the provided contract code database.
func WriteCode(db kvdb.KeyValueWriter, hash common.Hash, code []byte) {
...
func DeleteCode(db kvdb.KeyValueWriter, hash common.Hash) {
...
func ReadStateID(db kvdb.KeyValueReader, root common.Hash) *uint64 {
...
func WriteStateID(db kvdb.KeyValueWriter, root common.Hash, id uint64) {
	var buff [8]byte
...
func DeleteStateID(db kvdb.KeyValueWriter, root common.Hash) {
...
func ReadPersistentStateID(db kvdb.KeyValueReader) uint64 {
...
func WritePersistentStateID(db kvdb.KeyValueWriter, number uint64) {
...
// the last shutdown.
func ReadTrieJournal(db kvdb.KeyValueReader) []byte {
...
// shutdown.
func WriteTrieJournal(db kvdb.KeyValueWriter, journal []byte) {
...
// the last shutdown.
func DeleteTrieJournal(db kvdb.KeyValueWriter) {
...
// state).
func ReadStateHistoryMeta(db kvdb.AncientReaderOp, id uint64) []byte {
...
// state).
func ReadStateHistoryMetaList(db kvdb.AncientReaderOp, start uint64, count uint64) ([][]byte, error) {
...
func ReadStateAccountIndex(db kvdb.AncientReaderOp, id uint64) []byte {
...
// state history. Compute the position of state history in freezer by minus one
// since the id of first state history starts from one(zero for initial state).
func ReadStateStorageIndex(db kvdb.AncientReaderOp, id uint64) []byte {
...
// state history. Compute the position of state history in freezer by minus one
// since the id of first state history starts from one(zero for initial state).
func ReadStateAccountHistory(db kvdb.AncientReaderOp, id uint64) []byte {
...
// state history. Compute the position of state history in freezer by minus one
// since the id of first state history starts from one(zero for initial state).
func ReadStateStorageHistory(db kvdb.AncientReaderOp, id uint64) []byte {
...
func ReadStateHistory(db kvdb.AncientReaderOp, id uint64) ([]byte, []byte, []byte, []byte, []byte, error) {
...
func WriteStateHistory(db kvdb.AncientWriter, id uint64, meta []byte, accountIndex []byte, storageIndex []byte, accounts []byte, storages []byte) {
	db.ModifyAncients(func(op kvdb.AncientWriteOp) error {
