...
func ReadSkeletonSyncStatus(db kvdb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db kvdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db kvdb.KeyValueWriter) {
...
func ReadSkeletonHeader(db kvdb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db kvdb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db kvdb.KeyValueWriter, number uint64) {
...
func ReadSnapSyncStatusFlag(db kvdb.KeyValueReader) uint8 {
...
func WriteSnapSyncStatusFlag(db kvdb.KeyValueWriter, flag uint8) {
