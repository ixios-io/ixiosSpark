...
func (f *ResettableFreezer) ReadAncients(fn func(kvdb.AncientReaderOp) error) (err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *ResettableFreezer) ModifyAncients(fn func(kvdb.AncientWriteOp) error) (writeSize int64, err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
