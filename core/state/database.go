...
import (
	...
	"github.com/ixios-io/ixiosSpark/trie/trienode"
...
type Database interface {
	...
	DiskDB() kvdb.KeyValueStore

	// TrieDB returns the underlying trie database for managing trie nodes.
	TrieDB() *triedb.Database
}

// Trie is a Ixios Merkle Patricia trie.
type Trie interface {
	...
	Prove(key []byte, proofDb kvdb.KeyValueWriter) error
}
...
// large memory cache.
func NewDatabaseWithConfig(db kvdb.Database, config *triedb.Config) Database {
	return &cachingDB{
		disk:          db,
...
func NewDatabaseWithNodeDB(db kvdb.Database, triedb *triedb.Database) Database {
	return &cachingDB{
		disk:          db,
		codeSizeCache: lru.NewCache[common.Hash, int](codeSizeCacheSize),
		codeCache:     lru.NewSizeConstrainedCache[common.Hash, []byte](codeCacheSize),
...
func (db *cachingDB) OpenStorageTrie(stateRoot common.Hash, address common.Address, root common.Hash, self Trie) (Trie, error) {
	...
	tr, err := trie.NewStateTrie(trie.StorageTrieID(stateRoot, crypto.Keccak256Hash(address.CompactBytes()), root), db.triedb)
	if err != nil {
		return nil, err
	}
	return tr, nil
...
// DiskDB returns the underlying key-value disk database.
func (db *cachingDB) DiskDB() kvdb.KeyValueStore {
	return db.disk
}
