...
func NewPruner(db kvdb.Database, config Config) (*Pruner, error) {
	...
	if config.BloomSize < 768 {
		log.Warn("Sanitizing bloomfilter size", "provided(MB)", config.BloomSize, "updated(MB)", 768)
		config.BloomSize = 768
	}
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb kvdb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db kvdb.Database) error {
...
func extractGenesis(db kvdb.Database, stateBloom *stateBloom) error {
