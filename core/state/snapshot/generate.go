...
func generateSnapshot(diskdb kvdb.KeyValueStore, triedb *triedb.Database, cache int, root common.Hash) *diskLayer {
...
func journalProgress(db kvdb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (dl *diskLayer) generateRange(ctx *generatorContext, trieId *trie.ID, prefix []byte, kind string, origin []byte, max int, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	if origin == nil && last == nil {
		log.Error("entire trie is missing")
	}
