...
func CheckDanglingStorage(chaindb kvdb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb kvdb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db kvdb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db kvdb.KeyValueStore, hash common.Hash) error {
