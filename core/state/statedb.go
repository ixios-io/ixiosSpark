...
func (s *StateDB) Copy() *StateDB {
	...
	state := &StateDB{
		...
		// sealer to operate trie-backed only.
		snaps: s.snaps,
		snap:  s.snap,
	}
	// Copy the dirty states, logs, and preimages
	for addr := range s.journal.dirties {
		// As documented [here](https://github.com/ixios-io/ixiosSpark/pull/16485#issuecomment-380438527),
...
func (s *StateDB) deleteStorage(addr common.Address, addrHash common.Hash, root common.Hash) (bool, map[common.Hash][]byte, *trienode.NodeSet, error) {
	...
	// one just in case.
	if s.snap != nil {
		aborted, _, slots, nodes, err = s.fastDeleteStorage(addrHash, root)
	}
	if s.snap == nil || err != nil {
		aborted, _, slots, nodes, err = s.slowDeleteStorage(addr, addrHash, root)
	}
	if err != nil {
...
func (s *StateDB) handleDestruction(nodes *trienode.MergedNodeSet) (map[common.Address]struct{}, error) {
	// Short circuit if ixiosSpark is running with hash mode. This procedure can consume
