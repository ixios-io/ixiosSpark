...
//     commit to their own current state, which is independent of Ixios mainnet
//     (state, txs). This means that there's no reason for blob tx cancellation or
//     replacement, apart from a potential basefee / sealer tip adjustment.
//
...
//     that cancel blobs, blocks might contain such transaction (malicious sealer
...
//     the sealer tip meaningfully as a splitter.
//
...
func (p *BlobPool) Init(gasTip uint64, head *types.Header, reserve txpool.AddressReserver) error {
	...
	p.evict = newPriceHeap(uint256.NewInt(0), blobfee, &p.index)
...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	p.evict.reinit(uint256.NewInt(0), blobfee, false)
...
func (p *BlobPool) SetGasTip(tip *big.Int) {
	...
	// If the min sealer fee increased, remove transactions below the new threshold
