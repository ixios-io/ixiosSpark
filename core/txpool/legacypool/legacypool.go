...
const (
	...
	// addToPoolTimeout is the maximum allowable time for transactions to be added to the pool
	addToPoolTimeout = 5 * time.Second
)
...
var DefaultConfig = Config{
	...
	Rejournal: 30 * time.Minute,

	PriceLimit: 1,
	PriceBump:  10,

	AccountSlots: 128,
	GlobalSlots:  16384, // urgent + floating queue capacity with 4:1 ratio
	AccountQueue: 128,
	GlobalQueue:  8192,
...
// TimeoutRWMutex is a read–write mutex with timeout support.
// It uses a weighted semaphore to allow up to maxReaders concurrent readers.
// A writer must acquire all tokens to exclude readers.
...
// maxReaders determines the maximum number of concurrent readers allowed.
// A typical value might be 1000.
func NewTimeoutRWMutex(maxReaders int64) *TimeoutRWMutex {
	return &TimeoutRWMutex{
		sem: semaphore.NewWeighted(maxReaders),
		max: maxReaders,
	}
}

// RLockWithTimeout attempts to acquire a read lock within the given timeout.
// It returns an error if the timeout expires before the lock is acquired.
func (m *TimeoutRWMutex) RLockWithTimeout(timeout time.Duration) error {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	if err := m.sem.Acquire(ctx, 1); err != nil {
		return errors.New("timeout acquiring read lock")
	}
	return nil
}
...
// LockWithTimeout attempts to acquire a write lock within the given timeout.
// It does so by acquiring all available tokens.
// It returns an error if the timeout expires before the lock is acquired.
func (m *TimeoutRWMutex) LockWithTimeout(timeout time.Duration) error {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	if err := m.sem.Acquire(ctx, m.max); err != nil {
		return errors.New("timeout acquiring write lock")
	}
	return nil
}
...
func (pool *LegacyPool) SetGasTip(tip *big.Int) {
	// Lock and unlock not needed, minimum gas tip is *always* zero
	//pool.mu.Lock()
	//defer pool.mu.Unlock()

	pool.gasTip.Store(uint256.NewInt(0)) // Minimum gas tip is always 0
	return
}
...
func (pool *LegacyPool) Pending(filter txpool.PendingFilter) map[common.Address][]*txpool.LazyTransaction {
	...
	for addr, list := range pool.pending {
		...
		// If the sealer requests tip enforcement, cap the lists now
...
func (pool *LegacyPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if err := txpool.ValidateTransactionWithState(tx, pool.signer, opts); err != nil {
		log.Debug("ValidateTransactionWithState failed", "err", err)
		return err
	}
	return nil
}
...
func (pool *LegacyPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	log.Trace("legacypool journalTx start")
	pool.journalTx(from, tx)
	log.Trace("legacypool journalTx end")
...
func (pool *LegacyPool) Add(txs []*types.Transaction, local, sync bool) []error {
	...
	for i, tx = range txs {
		...
		if pool.all.Get(tx.Hash()) != nil {
			log.Debug("txpool.ErrAlreadyKnown", "txhash", tx.Hash().String())
	...
	timeoutTimer := time.NewTimer(addToPoolTimeout)
	defer timeoutTimer.Stop()
	if sync {
		select {
		case <-done:
		case <-timeoutTimer.C:
			log.Error("LegacyPool timeout; failed to add transaction(s)", "numTransactions", len(txs))
			return errs
		}
	}
	return errs
