...
// Header represents a block header in the Ixios blockchain.
type Header struct {
	ParentHash  common.Hash    `json:"parentHash"       gencodec:"required"`
	OmmerHash   common.Hash    `json:"sha3Uncles"       gencodec:"required"`
	Coinbase    common.Address `json:"sealer"`
...
func (h *Header) SanityCheck() error {
	...
	if eLen := len(h.Extra); eLen > 838_860_800 {
...
func (h *Header) EmptyBody() bool {
	...
	return h.TxHash == EmptyTxsHash && h.OmmerHash == EmptyOmmerHash
}
...
// Block represents an Ixios block.
//
...
type Block struct {
	...
	// These fields are used by package ixios to track
...
//
// The values of TxHash, OmmerHash, ReceiptHash and Bloom in header
// are ignored and set to values derived from the given txs, uncles
// and receipts.
func NewBlock(header *Header, txs []*Transaction, uncles []*Header, receipts []*Receipt, hasher TrieHasher) *Block {
	b := &Block{
		header:     CopyHeader(header),
		ReceivedAt: time.Now(), // Initialize ReceivedAt
	}
	...
	if len(uncles) == 0 {
		b.header.OmmerHash = EmptyOmmerHash
	} else {
		b.header.OmmerHash = CalcOmmerHash(uncles)
...
//
// The values of TxHash, OmmerHash, ReceiptHash and Bloom in header are ignored and set to
...
func (b *Block) OmmerHash() common.Hash   { return b.header.OmmerHash }
...
func CalcOmmerHash(uncles []*Header) common.Hash {
	if len(uncles) == 0 {
		return EmptyOmmerHash
	}
