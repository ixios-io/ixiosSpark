...
// Modified by ixiosSpark Authors
// Use of this source code is governed by a BSD-style license that can be found in
// the LICENSE file.
// Relicensed to BSD-3 via https://archive.is/MPBqK
...
func checkSignature(sig []byte) error {
	...
	case 65:
		...
		return nil
	case 7188:
		// Check recid in the first 65 bytes
		if sig[64] >= 4 {
			return ErrInvalidRecoveryID
		}
		...
		for i := 65; i < 7188; i++ {
			if sig[i] != 0 {
				return errors.New("Dilthium5 not implemented")
			}
		}
		return nil
	default:
