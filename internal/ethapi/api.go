...
import (
	...
	"github.com/ixios-io/ixiosSpark/ixios/gasestimator"
	"github.com/ixios-io/ixiosSpark/ixios/tracers/logger"
...
// IxiosAPI provides an API to access Ixios related information.
type IxiosAPI struct {
	b Backend
}

// NewIxiosAPI creates a new Ixios protocol API.
func NewIxiosAPI(b Backend) *IxiosAPI {
	return &IxiosAPI{b}
}
...
func (s *IxiosAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *IxiosAPI) FeeHistory(ctx context.Context, blockCount math.HexOrDecimal64, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *IxiosAPI) Syncing() (interface{}, error) {
...
// IxiosAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type IxiosAccountAPI struct {
	am *accounts.Manager
}

func NewIxiosAccountAPI(am *accounts.Manager) *IxiosAccountAPI {
	return &IxiosAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *IxiosAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Ixios ECDSA signature for:
// keccak256("\x19Ixios Signed Message:\n" + len(message) + message))
//
...
//
// https://gibit.Ixios.org/docs/interacting-with-geth/rpc/ns-personal#personal-sign
...
//
// https://gibit.Ethereum.org/docs/interacting-with-geth/rpc/ns-personal#personal-ecrecover
...
// Unpair deletes a pairing between wallet and ixiosSpark.
...
// proofList implements kvdb.KeyValueWriter and collects the proofs as
...
func (s *BlockChainAPI) GetHeaderByNumber(ctx context.Context, number rpc.BlockNumber) (map[string]interface{}, error) {
	...
	if header != nil && err == nil {
		...
		if number == rpc.PendingBlockNumber {
			...
			for _, field := range []string{"hash", "nonce", "sealer"} {
				response[field] = nil
			}
		}
		return response, err
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	result := map[string]interface{}{
		...
		"sha3Uncles":       head.OmmerHash,
		"logsBloom":        head.Bloom,
		"stateRoot":        head.Root,
		"sealer":           head.Coinbase,
...
func NewRPCPendingTransaction(tx *types.Transaction, current *types.Header, config *params.ChainConfig) *RPCTransaction {
	...
	if current != nil {
		baseFee = new(big.Int) // EIP1559 disabled
