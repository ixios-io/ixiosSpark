...
const ourPath = "github.com/ixios-io/ixiosSpark" // Path to our module
...
// conventions in the Ixios p2p network.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "ixiosSpark".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("ixiosSpark %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("ixiosSpark %s", params.VersionWithMeta)
	}
