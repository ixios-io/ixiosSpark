...
// DebugAPI is the collection of Ixios full node APIs for debugging the
// protocol.
...
func NewDebugAPI(eth *Ixios) *DebugAPI {
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		// the sealer and operate on those
		_, stateDb := api.eth.sealer.Pending()
		if stateDb == nil {
...
func storageRangeAt(statedb *state.StateDB, root common.Hash, address common.Address, start []byte, maxResult int) (StorageRangeResult, error) {
	...
	id := trie.StorageTrieID(root, crypto.Keccak256Hash(address.CompactBytes()), storageRoot)
