...
import (
	...
	"github.com/ixios-io/ixiosSpark/consensus/engine"
	...
	"github.com/ixios-io/ixiosSpark/params/forks"
	"github.com/ixios-io/ixiosSpark/rpc"
	"github.com/ixios-io/ixiosSpark/sealer"
)

// Register adds the engine API to the full node.
func Register(stack *node.Node, backend *ixios.Ixios) error {
	log.Info("Engine API enabled", "protocol", "ixiosSpark")
...
func NewConsensusAPI(eth *ixios.Ixios) *ConsensusAPI {
...
func newConsensusAPIWithoutHeartbeat(eth *ixios.Ixios) *ConsensusAPI {
	if eth.BlockChain().Config().TerminalTotalDifficulty != nil {
		log.Warn("Engine API started chain is configured for a merge")
	}
	api := &ConsensusAPI{
		eth:               eth,
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "ixiosSpark", ttd, "beacon", config.TerminalTotalDifficulty)
