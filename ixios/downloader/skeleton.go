...
//
// Opposed to the original Ixios block synchronization which is trustless (and
...
// not from within (vs. legacy Ixios sync).
//
...
type skeleton struct {
	db     kvdb.Database // Database backing the skeleton
...
func newSkeleton(db kvdb.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) saveSyncStatus(db kvdb.KeyValueWriter) {
