...
const (
	...
	fetchTimeout  = 2 * time.Second        // Maximum allotted time to return an explicitly requested block/transaction
)

const (
	maxUncleDist = 32    // Maximum allowed backward distance from the chain head
	maxQueueDist = 64    // Maximum allowed distance from the chain head to queue
	hashLimit    = 10240 // Maximum number of unique blocks or headers a peer may have announced
...
// the network.
func (f *BlockFetcher) Notify(peer string, hash common.Hash, number uint64, _time time.Time,
	headerFetcher headerRequesterFn, bodyFetcher bodyRequesterFn) error {

	now := time.Now()
	microTime := now.Unix()*1000000 + int64(now.Nanosecond())/1000
	...
	case f.notify <- block:
		microTime = now.Unix()*1000000 + int64(now.Nanosecond())/1000
		log.Debug("Block announcement accepted", "peer", peer, "hash", hash.Hex(), "number", number)
		return nil
	case <-f.quit:
		microTime = now.Unix()*1000000 + int64(now.Nanosecond())/1000
		log.Error("Block announcement rejected - fetcher quitting",
			microTime, peer, hash.Hex(), number)
...
func (f *BlockFetcher) loop() {
	...
	for {
		...
		for !f.queue.Empty() {
			...
			go f.importBlocks(op.origin, op.block) // note: goroutine added
		}
					...
					for _, hash := range hashes {
						go func(hash common.Hash) {
							...
							case <-timeout.C:
								...
								log.Error("Timeout reached while fetching headers", "peer", peer, "hash", hash)
								f.dropPeer(peer)
							}
						}(hash)
					}
			...
			for peer, hashes := range request {
				...
				go func(peer string, hashes []common.Hash) {
					...
					case <-timeout.C:
						...
						// waiting for a catchup. With an unresponsive
						// peer however, it's a protocol violation.
						log.Error("Timeout reached while fetching block body", "peer", peer)
						f.dropPeer(peer)
					}
				}(peer, hashes)
			}
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil {
						...
						if header.TxHash == types.EmptyTxsHash && header.OmmerHash == types.EmptyOmmerHash {
				...
				for i := 0; i < len(task.transactions) && i < len(task.uncles); i++ {
					...
					for hash, announce := range f.completing {
						...
						if uncleHash == (common.Hash{}) {
							uncleHash = types.CalcOmmerHash(task.uncles[i])
						}
						if uncleHash != announce.header.OmmerHash {
							continue
						}
...
func (f *BlockFetcher) rescheduleFetch(fetch *time.Timer) {
	...
	// Find the earliest expiring announcement
	earliest := time.Now()
...
func (f *BlockFetcher) importBlocks(peer string, block *types.Block) {
	...
	go func() {
		...
		case consensus.ErrFutureBlock:
			...
			log.Error("Weird future block", peer, "number", block.Number(), "hash", hash, "parent", block.ParentHash())
		...
		log.Info("Successful block import", "number", block.Number(), "hash", hash)
