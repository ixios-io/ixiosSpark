...
const sampleNumber = 8 // Number of transactions sampled in a block

var (
	DefaultMaxPrice    = big.NewInt(1).Mul(big.NewInt(10), big.NewInt(params.Ixios))
	DefaultIgnorePrice = big.NewInt(10000000000 * params.Wei)
)
...
func NewOracle(backend OracleBackend, params Config) *Oracle {
	...
	if blocks < 1 {
		blocks = 1
		log.Warn("Sanitising invalid gasprice oracle sample blocks", "provided", params.Blocks, "updated", blocks)
	}
	...
	if percent < 0 {
		...
		percent = 100
		log.Warn("Sanitising invalid gasprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
	}
	maxPrice := params.MaxPrice
	if maxPrice == nil || maxPrice.Cmp(big.NewInt(0)) < 0 {
		maxPrice = DefaultMaxPrice
		log.Warn("Sanitising invalid gasprice oracle price cap", "provided", params.MaxPrice, "updated", maxPrice)
	}
	ignorePrice := params.IgnorePrice
	if ignorePrice == nil || ignorePrice.Cmp(big.NewInt(0)) < 0 {
		ignorePrice = DefaultIgnorePrice
		log.Warn("Sanitising invalid gasprice oracle ignore price", "provided", params.IgnorePrice, "updated", ignorePrice)
	...
	if maxHeaderHistory < 1 {
		maxHeaderHistory = 1
		log.Warn("Sanitising invalid gasprice oracle max header history", "provided", params.MaxHeaderHistory, "updated", maxHeaderHistory)
	}
	maxBlockHistory := params.MaxBlockHistory
	if maxBlockHistory < 1 {
		maxBlockHistory = 1
		log.Warn("Sanitising invalid gasprice oracle max block history", "provided", params.MaxBlockHistory, "updated", maxBlockHistory)
	}
...
func (oracle *Oracle) SuggestTipCap(ctx context.Context) (*big.Int, error) {
	...
	for exp > 0 {
		...
		// - The block is empty
		// - All the transactions included are sent by the sealer itself.
...
// are sent by the sealer itself(it doesn't make any sense to include this kind of
