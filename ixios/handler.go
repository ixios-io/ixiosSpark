...
import (
	...
	"github.com/ixios-io/ixiosSpark/core/forkid"
	...
	"github.com/ixios-io/ixiosSpark/ixios/fetcher"
...
// support all the operations needed by the Ixios chain protocols.
...
type handlerConfig struct {
	Database       kvdb.Database          // Database for direct sync insertions
...
// newHandler returns a handler for all Ixios chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("IxiosSpark handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("IxiosSpark peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer); err != nil {
		peer.Log().Error("IxiosSpark peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Ixios L1 peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Ixios L1 peer", "", nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Ixios L1 peer removal failed", "err", err)
	}
}

func (h *handler) Start(maxPeers int, enableBroadcast bool) {
	h.maxPeers = maxPeers
	h.enableBroadcast = enableBroadcast
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	...
	// If the block is indeed in our own chain, announce it
	if h.chain.HasBlock(hash, block.NumberU64()) {
		for _, peer := range peers {
			go peer.AsyncSendNewBlockHash(block) // goroutine added
		}
	}
	...
	if propagate {
		...
		if h.enableBroadcast {
			// Send the block to all of our peers
			transfer = peers
		} else {
		...
		for _, peer := range transfer {
			go peer.AsyncSendNewBlock(block, td)
		}
...
func (h *handler) minedBroadcastLoop() {
	...
	for obj := range h.minedBlockSub.Chan() {
		if ev, ok := obj.Data.(core.NewMinedBlockEvent); ok {
			ev.Block.ReceivedAt = time.Now()  // Set timestamp for locally mined blocks
