...
func (p *Peer) broadcastBlocks() {
	for {
		...
		case prop := <-p.queuedBlocks:
			...
			log.Info("Propagated block", "number", prop.block.Number(), "hash", prop.block.Hash())
			if err != nil {
				log.Error("Failed to propogate block", "err", err.Error(), "number", prop.block.Number(), "hash", prop.block.Hash())
			}

		case block := <-p.queuedBlockAnns:
			err := p.SendNewBlockHashes([]common.Hash{block.Hash()}, []uint64{block.NumberU64()})
			p.Log().Debug("Announced block", "number", block.Number(), "hash", block.Hash())
			if err != nil {
