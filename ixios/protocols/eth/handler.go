...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Ixios L1 network ID
...
// Handle is invoked whenever a connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed", "remoteAddr", peer.RemoteAddr(), "err", err)
			return err
		}
	}
}
...
var ixiosMessage = map[uint64]msgHandler{
