...
func New(file string, cache int, handles int, namespace string, readonly bool, ephemeral bool) (*Database, error) {
	...
	// Disable seek compaction explicitly. Check https://archive.is/YfcCW for more details.
...
func (d *Database) Close() error {
	if !d.quitLock.TryLock() {
		return errors.New("pebble.Close() failed to acquire lock")
	}
	defer d.quitLock.Unlock()

	// allow double closing; if already closed, return immediately.
	if d.closed {
		return nil
	}
	d.closed = true

	// If a quit channel exists, try to notify it.
	if d.quitChan != nil {
		errc := make(chan error)
		// Use a non-blocking send so that if no receiver is waiting, we don't block forever.
		select {
		case d.quitChan <- errc:
			// Successfully sent errc
		default:
			// No receiver is ready on d.quitChan; skip sending.
		}
		d.quitChan = nil
	}

	// Close the underlying database and return any error it produces.
	return d.db.Close()
}
...
func (d *Database) NewBatch() kvdb.Batch {
	return &batch{
		b:  d.db.NewBatch(),
		db: d,
	}
...
func (d *Database) NewBatchWithSize(size int) kvdb.Batch {
	return &batch{
...
func (d *Database) NewSnapshot() (kvdb.Snapshot, error) {
...
func (b *batch) Replay(w kvdb.KeyValueWriter) error {
	...
	for {
		...
		if err != nil {
			fmt.Errorf("pebble: read error: %v", err)
			break
		}
		if !ok {
			break
...
func (d *Database) NewIterator(prefix []byte, start []byte) kvdb.Iterator {
