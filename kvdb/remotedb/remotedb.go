...
// Package remotedb implements the key-value database layer based on a remote ixiosSpark
// node. Under the hood, it utilises the `debug_dbGet` method to implement a
// read-only database.
// There really are no guarantees in this database, since the local node does not have
...
func (db *Database) ReadAncients(fn func(op kvdb.AncientReaderOp) error) (err error) {
	return fn(db)
}
...
func (db *Database) ModifyAncients(f func(kvdb.AncientWriteOp) error) (int64, error) {
	panic("not supported")
}
...
func New(client *rpc.Client) kvdb.Database {
	return &Database{
		remote: client,
	}
}
