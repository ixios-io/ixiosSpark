...
// Copyright 2025 The ixiosSpark Authors, Copyright 2014-2024 The go-ethereum Authors (geth)
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of ixiosSpark is "ixiosSpark". If no
...
func (c *Config) NodeName() string {
	...
	if name == "ixiosSpark" || name == "ixiosSpark-testnet" {
		name = "ixiosSpark"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "ixiosSpark" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by legacy versions are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		oldpath := ""
		if c.name() == "ixiosSpark" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				c.oldGethResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'ixiosSpark' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
