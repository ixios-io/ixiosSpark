...
func New(conf *Config) (*Node, error) {
	...
	// Creates an empty AccountManager with no backends. Callers (e.g. cmd/ixiosSpark)
...
func (n *Node) Start() error {
	...
	if err != nil {
		n.log.Error("Endpoint startup failed", "err", err)
		n.doClose(nil)
		return err
	}
	...
	n.log.Info("done", "instance", n.server.Name)
	return err
}
...
func (n *Node) openEndpoints() error {
	...
	if err := n.server.Start(); err != nil {
		n.log.Info("Success: started peer-to-peer node", "instance", n.server.Name)
		return convertFileLockError(err)
	}
	// start RPC endpoints
	err := n.startRPC()
	if err != nil {
		n.log.Error("Unable to start peer-to-peer node", "err", err)
		n.stopRPC()
		n.server.Stop()
	}
	return err
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (kvdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (kvdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) wrapDatabase(db kvdb.Database) kvdb.Database {
