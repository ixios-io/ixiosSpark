...
import (
	...
	"github.com/ixios-io/ixiosSpark/p2p/discover"
	"github.com/ixios-io/ixiosSpark/p2p/enode"
	"github.com/ixios-io/ixiosSpark/p2p/enr"
...
const (
	defaultDialTimeout = 850 * time.Millisecond
	...
	defaultMaxPendingPeers = 128
	defaultDialRatio       = 64

	// This time limits inbound connection attempts per source IP.
	inboundThrottleTime = 350 * time.Millisecond

	// Maximum time allowed for reading a complete message.
	// This is effectively the amount of time a connection can be idle.
	frameReadTimeout = 1500 * time.Millisecond

	// Maximum amount of time allowed for writing a complete message.
	frameWriteTimeout = 850 * time.Millisecond
)
...
type Config struct {
	...
	// Enable broadcasting to all peers
...
func (srv *Server) Self() *enode.Node {
	if !srv.lock.TryLock() {
		log.Error("Failed to acquire lock for Self()")
		return nil
	}
	ln := srv.localnode
	srv.lock.Unlock()
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // Ixios Node Record
