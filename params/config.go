...
type GuildioConfig struct {
	Period    uint64 `json:"period"` // Number of seconds between blocks to enforce
	Epoch     uint64 `json:"epoch"`  // Epoch length to reset votes and checkpoint
	WarpPoint uint64 `json:"warpPoint"`
}
...
func (c *ChainConfig) Description() string {
	...
	switch {
	case c.Ethash != nil:
		fmt.Printf("Error: Ethash is disabled")
		os.Exit(1)
	case c.Clique != nil:
		if c.TerminalTotalDifficulty == nil {
			banner += "Consensus: FastClique\n"
	...
	if c.ChainID.String() == MainnetChainConfig.ChainID.String() {
...
// CheckConfigForkOrder checks that we don't "skip" any forks, ixiosSpark isn't pluggable enough
