...
// Custom decoder for handling hex strings without 0x prefix for Address types
type addressDecoder struct {
	address *common.Address
}

func (d *addressDecoder) UnmarshalJSON(data []byte) error {
	var hexStr string
	...
	if !strings.HasPrefix(hexStr, "0x") && len(hexStr) == 40 {
		// Verify it's a valid hex string first
		if _, err := hex.DecodeString(hexStr); err == nil {
			hexStr = "0x" + hexStr
		}
	}

	// Use existing method to convert hex to address
	*d.address = common.HexToAddress(hexStr)
	return nil
}
...
func parseArgumentArray(dec *json.Decoder, types []reflect.Type) ([]reflect.Value, error) {
	...
	for i := 0; dec.More(); i++ {
		...
		// Check if type is common.Address
		if types[i] == reflect.TypeOf(common.Address{}) {
			// Use custom decoder for addresses
			var addr common.Address
			decoder := &addressDecoder{address: &addr}
		...
		// Regular decoding for other types
