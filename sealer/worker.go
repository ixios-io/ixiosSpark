...
const (
	...
	// any newly arrived transactions.
	maxRecommitInterval = 1250 * time.Millisecond
...
type worker struct {
	...
	// External functions
	isLocalBlock func(header *types.Header) bool // Function used to determine whether the specified block is mined by local sealer.
...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(header *types.Header) bool, init bool) *worker {
	...
	if recommit < minRecommitInterval {
		log.Warn("Sanitizing sealer recommit interval", "provided", recommit, "updated", minRecommitInterval)
...
// setGasTip sets the minimum sealer tip needed to include a non-local transaction.
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case req := <-w.newWorkCh:
			log.Debug("Preparing for new block (commitWork)")
...
func (w *worker) resultLoop() {
	...
	for {
		...
		case block := <-w.resultCh:
			...
			if err != nil {
				log.Error("Failed posting block to chain", "err", err)
			}
...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address) (*environment, error) {
	...
	// the sealer to speed block sealing up a bit.
...
func (w *worker) commitBlobTransaction(env *environment, tx *types.Transaction) ([]*types.Log, error) {
	sc := tx.BlobTxSidecar()
	if sc == nil {
		panic("blob transaction without blobs in sealer")
	}
...
func (w *worker) commitTransactions(env *environment, plainTxs, blobTxs *transactionsByPriceAndNonce, interrupt *atomic.Int32) error {
	...
	for {
		...
		if txCount >= int(params.MaxTransactionsPerBlock) {
			log.Trace("Block transaction limit reached", "count", txCount)
			break
		}
		...
		case errors.Is(err, core.ErrNonceTooLow):
			// New head notification data race between the transaction pool and sealer, shift
	...
	if !w.isRunning() && len(coalescedLogs) > 0 {
		...
		// logs by filling in the block hash when the block was mined by the local sealer. This can
...
func (w *worker) fillTransactions(interrupt *atomic.Int32, env *environment) error {
	...
	filter := txpool.PendingFilter{
		MinTip: uint256.NewInt(0), // Set minimum tip to always be zero
	}
...
func (w *worker) commitWork(interrupt *atomic.Int32, timestamp int64) {
	...
	case errors.Is(err, errBlockInterruptedByNewHead):
		...
		// delay, and possibly causes sealer to mine on the previous head,
		// which could result in higher uncle rate.
		work.discard()
		log.Info("Discarding commit due to new head", "number", w.chain.CurrentBlock().Number)
		return
	}
	// Submit the generated block for consensus sealing.
	err = w.commit(work.copy(), w.fullTaskHook, true, start)
	if err != nil {
		log.Error("failed to commit sealed block", "err", err)
	}
	...
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: env.receipts, state: env.state, block: block, createdAt: time.Now()}:
			log.Debug("Commit new sealing work", "number", block.Number(), "sealhash", w.engine.SealHash(block.Header()),
				"txs", env.tcount, "gas", block.GasUsed(),
				"elapsed", common.PrettyDuration(time.Since(start)), "signer", block.Coinbase())
...
func (w *worker) getSealingBlock(params *generateParams) *newPayloadResult {
	...
	case w.getWorkCh <- req:
		return <-req.result
	case <-w.exitCh:
		return &newPayloadResult{err: errors.New("sealer closed")}
	}
}
...
// totalFees computes total consumed sealer fees in Wei. Block transactions and receipts have to have the same order.
func totalFees(block *types.Block, receipts []*types.Receipt) *big.Int {
	feesWei := new(big.Int)
	for i, tx := range block.Transactions() {
		sealerFee, _ := tx.EffectiveGasTip(block.BaseFee())
		feesWei.Add(feesWei, new(big.Int).Mul(new(big.Int).SetUint64(receipts[i].GasUsed), sealerFee))
	}
	return feesWei
}
