...
import (
	...
	"github.com/ixios-io/ixiosSpark/signer/core/apitypes"
)
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req       *SignDataRequest
		useIxiosV = true // Default to use V = 27 or 28, the legacy Ixios format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useIxiosV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// FastClique is the Ixios PoA standard
		...
		newExtra := make([]byte, len(header.Extra)+7188)
		...
		messages := []*apitypes.NameValueType{
			{
				Name:  "FastClique header",
				Typ:   "clique",
				Value: fmt.Sprintf("fastclique header %d [%#x]", header.Number, header.Hash()),
			},
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Ixios ECDSA signature for:
		// hash = keccak256("\x19Ixios Signed Message:\n${message length}${message}")
...
func cliqueHeaderHashAndRlp(header *types.Header) (hash, rlp []byte, err error) {
	...
	rlp = fastClique.CliqueRLP(header)
	hash = fastClique.SealHash(header).Bytes()
	return hash, rlp, err
}
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	//
	// https://gibit.Ixios.org/docs/tools/clef/apis#account-ecrecover
	if len(sig) != 65 {

	}
	switch len(sig) {
	case 7188:
		// Verify everything after 65 bytes is zero
		for i := 65; i < 7188; i++ {
			if sig[i] != 0 {
				return common.Address{}, errors.New("Dilithium5 not implemented")
			}
		}
		break
	default:
		return common.Address{}, errors.New("signature must be 65 bytes or 7188 bytes")
	}

	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, errors.New("invalid Ixios signature (V is not 27 or 28)")
	}
